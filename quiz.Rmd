...
The R-squared of `model_crime` approximates 0.44, which can be improved upon, for example, by adding more predictor variables. One of the techniques for selecting predictor variables is using stepwise regression algorithm. Use the `step()` function with `direction="backward"` and store the best model under the `model_step` object.
